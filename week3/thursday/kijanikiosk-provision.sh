...
provision_logrotate() {
  log "=== Phase 5b: Log Rotation ==="

  # Write logrotate config for KijaniKiosk application logs
  # su directive required because shared/logs is SGID 2770 (group-writable, not root-owned)
...
/opt/kijanikiosk/shared/logs/*.log {
    ...
    sharedscripts
    postrotate
        # Signal the application to reopen log handles if needed
...
LOGROTATE
  ...
  log "  Rotation: daily, retention: 14 days, compression: enabled (delaycompress)"

  # Validate the config syntax (dry-run)
  if logrotate --debug /etc/logrotate.d/kijanikiosk >/dev/null 2>&1; then
    success "Phase 5b complete — logrotate configured and validated"
  else
    error "Logrotate config validation failed — check /etc/logrotate.d/kijanikiosk"
  fi
}
verify_state() {
  ...
  if [[ -f /etc/logrotate.d/kijanikiosk ]]; then
    success "Logrotate config exists: /etc/logrotate.d/kijanikiosk"
  else
    log "FAIL: Logrotate config missing: /etc/logrotate.d/kijanikiosk"
    ((failed++))
  fi
